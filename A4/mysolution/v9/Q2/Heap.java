...
public class Heap {
  ...
  private void buildHeapUtil(int[] keys_array, int[] values_array)
    ...
    // Initializing Root
    Node rootNode = new Node(keys_array[0], values_array[0], null);
    nodes_array[keys_array[0]] = rootNode;
    root = rootNode;
    recursiveBinaryTree(root, 0, keys_array, values_array);
    // Heapify -> Use Post Order Traversal
    heapifyPostOrder(root);
  }

  private void recursiveBinaryTree(
    Node root,
    int i,
    int[] keys_array,
    int[] values_array
  ) {
    if (2 * i + 1 >= keys_array.length) {
      return;
    }
    // Create left Child
    ...
    root.left = leftChild;
    nodes_array[keys_array[2 * i + 1]] = leftChild;

    if (2 * i + 2 >= keys_array.length) {
      return;
    }
    ...
    if (2 * i + 2 < keys_array.length) {
      ...
      nodes_array[keys_array[2 * i + 2]] = rightChild;
    }
    recursiveBinaryTree(root.left, 2 * i + 1, keys_array, values_array);
    recursiveBinaryTree(root.right, 2 * i + 2, keys_array, values_array);
  }

  private void heapifyPostOrder(Node root) {
    if (root == null) {
      return;
    }
    heapifyPostOrder(root.right);
    heapifyPostOrder(root.left);
    heapify(root);
  }
